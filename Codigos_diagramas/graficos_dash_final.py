...
try:
    ...
    app.layout = html.Div(children = [    
        ...
        html.Div([  
            html.H2(children='Integrantes: ', style={'font-family': 'Segoe UI', 'margin': '20px'}),
            html.Div(children='Santiago Cruz Roldán', style={'font-family': 'Segoe UI', 'margin': '20px', 'font-size':'20px'}),
            html.Div(children='Wilberson Osorio Rodríguez', style={'font-family': 'Segoe UI', 'margin': '20px', 'font-size':'20px'}),
            html.Div(children='Juan Sebastián Pacheco Velásquez', style={'font-family': 'Segoe UI', 'margin': '20px', 'font-size':'20px'}),
        ...
        html.Div([    
         ...
         html.Div([    
            ...
            html.P(children = '''(Santiago Cruz) Como podemos observar en la gráfica de casos por municipios, Barranquilla es el municipio con más casos registrados, albergando casi el 30% de casos positivos de Covid en la base de datos. Después de Barranquilla, las ciudades con más contagios fueron Medellín y Manizales, con 24 y 20 casos respectivamente, como podemos analizar, los tres municipios con más registros de covid en nuestra base de datos son ciudades capitales de sus respectivos departamentos, Atlántico, Antioquia y Caldas. (Wilberson Osorio) Que las tres sean capitales nos lleva a pensar en tres razones para que esto ocurra:
1. Mayor densidad de población: Las capitales suelen tener una mayor concentración de personas en comparación con las áreas rurales. Cuantas más personas estén en un área determinada, mayor será el potencial de transmisión del virus.
2. Movilidad y transporte: Las capitales son centros de actividad económica y social, lo que implica un mayor movimiento de personas a través de sistemas de transporte público y privado. 
3. Interacciones sociales: Las áreas urbanas suelen ser lugares donde las interacciones sociales son más frecuentes, ya sea en el trabajo, en establecimientos comerciales o en eventos culturales.
(Juan Pacheco) También nos damos cuenta que hay 36 municipios que solamente registraron de a un caso, siendo estos poco provechosos para el análisis ya que entre estos 36 municipios sólo representan el 12% de la base de datos.
Entre Barranquilla, Medellín, Manizales, Palmira y Jamundí se concentran exactamente el 50% de los datos.
         ...
         html.Div([    
            ...
            html.P(children = '''(Wilberson Osorio) Como podemos ver en el análisis, Atlántico, Antioquia , Valle del Cauca y Caldas albergan la mayoría de los casos, sumando un total de 226, representando el 75% de la base de datos. Algo que era de esperarse puesto que las ciudades de estos departamentos tuvieron una representación significativa en el análisis por municipio, entre los 4 departamentos con más casos aparecen los departamentos de las tres ciudades con más casos.
(Juan Pacheco) También nos damos cuenta que tan solo dos departamentos registraron solamente un caso, Córdoba y Sucre. 
Si bien en la base de datos no tenemos representación de todos los departamentos, tenemos una representación significativa, tenemos registros de 21 de un total de 32 departamentos pertenecientes a colombia, representando el 66% de los departamentos del paìs. Tenemos registros de los 17 departamentos más poblados del país, ordenados por número de habitantes de mayor a menor son:  Antioquia, Valle del Cauca, Cundinamarca, Atlántico, Santander, Bolívar, Córdoba, Norte de Santander, Nariño, Cauca, Magdalena, Cesar, Tolima, Boyacá, Huila, Meta, Caldas.
            ...
            html.Div([    
               ...
               html.P(children = '''(Santiago Cruz) Como podemos notar en la gráfica, el 54,3% de las personas registradas son mujeres, algo que aunque no parezca significativo sì lo es porque nos lleva a pensar en las razones por las cuales las mujeres sufrieron más contagios que los hombres.
Según el estudio realizado por la Organización Mundial de la Salud (OMS), al 28 de enero de 2021, los números globales eran los siguientes: 863,4 millones de casos confirmados de COVID-19, de los cuales 447,3 millones (51,6%) fueron mujeres y 416,1 millones (48,4%) fueron hombres.
                      ''',style={'font-family': 'Segoe UI', 'margin': '40px', 'font-size':'20px', 'text-align': 'justify'} ),  
               
                html.P(children = '''(Wilberson Osorio) Como podemos notar, las mujeres se contagiaron más que los hombres, 31 millones de mujeres más, esto se podría deber a que sus trabajos muchas veces les exigen un contacto cercano con otras personas, lo que significa que el riesgo de exposición al virus es mayor. Además, los roles de atención, tareas domésticas y familiares significan que muchas mujeres también tienen que viajar a menudo para atender a los miembros de la familia a pesar de la intensa transmisión del virus, y posiblemente tener una mayor vulnerabilidad al virus que los hombres debido a factores biológicos.
            ...
            html.Div([    
               ...
               html.P(children = '''
(Juan Pacheco) Podemos notar gracias a la gráfica que los datos se agrupan hacia el centro en un intervalo entre los 20 y 50 años aproximadamente. Podemos notar como antes de los 10 años y después de los 70 se notan muy pocos registros. 
Según la Organización Mundial de la Salud (OMS), el intervalo de edad que, hasta ahora, ha experimentado el mayor número de casos de COVID-19 es el de los adultos jóvenes de 20 a 40 años. (Santiago Cruz) Esta población es especialmente susceptible porque generalmente trabaja fuera de la casa, tiene el grupo más amplio de compañeros de trabajo y es más propensa a ir de fiesta y a viajar. Además, muchas personas de esta edad han tenido una escasa adherencia al distanciamiento social recomendado. Por lo que podemos ver la base de datos con la que estamos trabajando se asemeja bastante a los datos brindados por la OMS, por lo cual podemos suponer que la base de datos tiene buena representación.
            ...
            html.Div([    
               ...
               html.P(children = '''(Santiago Cruz) Podemos ver en la gráfica que solamente 9 personas tienen por estado del caso fallecidos, lo cual solo representa el 3% de la base de datos, mientras que el 96,3% de los datos son leves, lo cual indica un bajo porcentaje de mortalidad por covid en nuestra base de datos. Además, podemos relacionar la distribución por estado del caso con la distribución por edad, puesto que las personas fallecidas en la base de datos son la mayoría adultos mayores, teniendo un intervalo desde 58 años a 92 años, entre ese intervalo están las edades de las personas fallecidas. Podemos concluir entonces que la edad tiene una relación directa con las complicaciones que puede generar el covid en nuestros organismos.



                      ''',style={'font-family': 'Segoe UI', 'margin': '40px', 'font-size':'20px', 'text-align': 'justify'} ),  
               
                html.P(children = '''(Juan Pacheco) Se ha identificado que los mayores de 70 años han experimentado los resultados más graves. Esto se debe principalmente a que esta población es más susceptible a complicaciones relacionadas con el virus, como problemas respiratorios graves, enfermedades cardiovasculares, presión arterial alta o diabetes. La posibilidad de hospitalización es tres veces mayor para los mayores de 70 años que para los adultos jóvenes. Esto significa que una población más joven puede convertirse en una cadena de transmisión del virus para las personas mayores porque como pudimos ver en el análisis de distribución por edad los más contagiados son los adultos aunque estos no tienen las consecuencias más graves.
            ...
            html.Div([    
               ...
               html.P(children = '''(Wilberson Osorio) En esta distribución tenemos los valores de Indígena, palenquero, negro y otro que tienen respectivamente 13, 23, 37, 237 entradas. Esto nos muestra que el 79% de la población no se siente identificada con una etnia en específico. 
...
Las personas que se identifican con la comunidad indígena pertenecen a Atlántico, Valle del Cauca y Magdalena y representan el 9% de la base de datos. Las personas que se identifican como palenqueros pertenecen en su mayoría a Cesar, Caldas, Antioquia y Meta, siendo César el departamento con más registros de palenqueros 4/23. De las personas que se identifican como negros hay 20 que pertenecen al departamento del Atlántico que corresponden al 74% de las persona que se identifican como negros, complementando esto, Magdalena y Valle del Cauca tienen de a 2 registros.
            ...
            html.P(children = '''● (Santiago Cruz) En lo referente a la selección de las fuentes de datos. Es de reconocer que fue información la cual tuvo fácil acceso, ya que, se encontraba en la base de datos del gobierno, que se encuentra abierta para todos. He de recalcar, que los datos son obtenidos por el Ministerio de Salud. 
